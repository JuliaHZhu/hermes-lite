...
import pytest
from deck import Deck, build_deck, BASELINE_POOL
...
class TestDeck:
    ...
    def test_get_schemas_for_protocol_anthropic(self):
        ...
        assert "type" not in schemas[0]  # raw schema, not openai wrapper
...
class TestBuildDeck:
    def test_build_deck_adds_redundancy(self):
        ...
        assert d.size() == 3
        # Should be filled from BASELINE_POOL in order
    ...
    def test_build_deck_does_not_duplicate_skill_tools(self):
        ...
        # BASELINE_POOL[0] already in skill_tools, so redundancy fills from remaining
        assert BASELINE_POOL[0] in d.tools
        assert d.size() == 4  # 1 skill tool + 3 redundancy
    ...
    def test_build_deck_skips_missing_tools(self):
        r = make_registry([BASELINE_POOL[0]])  # only first baseline tool exists
