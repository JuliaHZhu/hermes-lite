...
class TestToolRegistry:
    ...
    def test_get_schemas_filtered(self):
        ...
        all_schemas = r.get_schemas()
        assert len(all_schemas) == 3

        math_only = r.get_schemas(tags=["math"])
        assert len(math_only) == 2

        enabled_only = r.get_schemas(enabled=["a", "b"])
    ...
    def test_get_tool_info_excludes_handler(self):
        r = ToolRegistry()
        r.register("info", "Info", {"properties": {}}, dummy_handler, tags=["t"], category="c")
        info = r.get_tool_info("info")
    ...
    def test_schema_caching(self):
        ...
        assert s1 == s2
        # Cache should be invalidated on register
        r.register("cached2", "C2", {"properties": {}}, dummy_handler)
        s3 = r.get_schemas(enabled=["cached"])
        assert len(s3) == 1  # still filtered correctly
    ...
    def test_cache_ttl_eviction(self):
        ...
        r.get_schemas()
        time.sleep(0.01)
        # After TTL, should rebuild (no crash)
...
def test_module_level_registry_has_default_tools():
    """After importing tools.*, the singleton registry should contain default tools."""
